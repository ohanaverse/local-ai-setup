...
instead of starting fresh. This lesson ports `find_latest_session` and
`compute_project_slug` from `wt-core.sh`.
...
| `git rev-list --max-parents=0 HEAD` | Finds the repo's root commit hash. |
...
```go
s, _ := session.LatestClaude("/some/worktree/path")
if s != nil {
...
(Or nothing, if there are no sessions.)

## Try It Yourself

Write a unit test for `Slug` that asserts
`Slug("/a/b/c.d") == "a-b-c-d"` (leading slash removed, dots → dashes).
