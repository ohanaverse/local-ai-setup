...
| `wt litellm sync [--json]` | Make local-model routes match the models actually running. A family (ollama, omlx, mtplx, mlx_lm_server) whose probe did not fully succeed is left alone (with a warning), and so are models whose provider has no probe (retired llamacpp; no warning). The exception is a server that *refuses the connection*: nothing is listening, so its routes are stale and are removed |
