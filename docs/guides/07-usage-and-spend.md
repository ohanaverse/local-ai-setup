...
- **wt launch history exists**: `/Users/keith/.config/agent-wt/usage.jsonl` — wt appends one JSON line per TUI launch (guide 06 §7). First line here:
  ...
  ```
- **modelman repo checked out** at `/Users/keith/github/ohanaverse/modelman`. Always invoke as `uv run modelman` from there — the PATH-installed `/Users/keith/.local/bin/modelman` predates the `usage` subcommand (Gotchas).
