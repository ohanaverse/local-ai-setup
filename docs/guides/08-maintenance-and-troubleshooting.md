...
- **Model missing from wt's picker, and it's a LOCAL model** (ollama/omlx/mtplx/mlx_lm_server): don't look at `exposed`/`ready` here — check whether it's actually **`running`** and probe-verified instead. Run `uv run modelman start <model_id>` (it starts the model if needed and reports success/failure), or check the TUI's RUNNING column. `exposed`/`ready` no longer gate local-model visibility in wt at all; a local model with `exposed = false` can still show up in wt's picker as long as it's running, and a local model with `exposed = true` but not running will NOT show up.
