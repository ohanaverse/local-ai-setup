...
**Last verified:** 2026-08-27, against `modelman`'s `git log`/`gh pr list`
directly (not from memory — status here goes stale fast; re-verify before
trusting it).
...
| Phase 2 PR 3 | Migrate `FamilyScreen` off globbing `families/*.yaml` onto `Registry.families()` | ⬜ Not yet planned or written — **next open step** |
| Phase 2 PR 4 | Clean up `app.py`'s `--initial-family` path off the legacy manifest; drop `MODELMAN_CONFIG`/`MODELMAN_FAMILY_DIR` env-vars | ⬜ Not yet planned |
