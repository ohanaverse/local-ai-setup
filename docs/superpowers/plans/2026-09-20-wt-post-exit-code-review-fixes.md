...
- **Finding 4 (`stopmodel_test.go:58`) — real, but a scope judgement, not a missing fixture.** `fakeEnv` replaces `e.backends`, so `TestStopModelSingleModelDelegatesToStop` proves only that dispatch reached a backend; a delegation stubbed to `return nil` would keep it green while the picker printed "done" having stopped nothing. The untested code is the one-line thunk `return b.stop(ctx, e, cfg)` shared by the omlx and mtplx backends, and its delegate is already covered — `backends_test.go:207` (`TestOmlxStopWaitsForPortClose`) uses `testEnv()`, a stubbed `lookPath`/`run` and the `serveAt`/`chatHandler` HTTP helpers, so the fixture this bullet used to claim was missing does exist. That makes covering it a test-only change with its own moving parts, left for a follow-up rather than folded into this pass.
