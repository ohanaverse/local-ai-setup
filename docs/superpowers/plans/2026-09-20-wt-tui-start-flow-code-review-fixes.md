...
- [ ] **Step 1: Write the failing tests** (append to `table_refresh_test.go`; reuse its
...
- [ ] **Step 4: Migrate the callers this breaks** — `start_flow.go`'s `finishStart` `back()`
...
- [ ] **Step 6: Commit** — `fix(tui): refreshTable returns a Cmd; forward SetItems' filter cmd`
...
- [ ] **Step 1: Write the failing test** — `TestSuccessfulStartThenFailedLaunchRefreshesTable`
...
- [ ] **Step 2: Run to verify failure** — `go test ./internal/tui -run TestSuccessfulStartThenFailedLaunchRefreshesTable -v` — Expected: FAIL.
- [ ] **Step 3: Implement** (`app.go`) — the `no model selected`, `session check failed`
      and `launch failed` bail paths in `proceedToLaunch` return `m.refreshTable()`.
- [ ] **Step 4: Run to verify pass** — as Task 1 Step 5.
- [ ] **Step 5: Commit** — `fix(tui): refresh the picker when a started model fails to launch`

### Task 3: ctrl+c is the only escape while cancelling

Finding 2, per the adjudicated decision above.

- [ ] **Step 1: Rewrite the test** — `TestKeysDuringStartCancelThenQuit` becomes
      ...
      status `cancelled`.
- [ ] **Step 2: Run to verify failure** — `go test ./internal/tui -run TestKeysDuringStartCancelThenOnlyCtrlCQuits -v` — Expected: FAIL.
- [ ] **Step 3: Implement** (`start_flow.go`) — see the contribution request below;
      update `startingView`'s cancelling branch to advertise only `[ctrl+c] quit wt` and
      say teardown is in progress.
- [ ] **Step 4: Amend the spec** — `docs/superpowers/specs/2026-09-20-wt-tui-start-flow-design.md`
...
- [ ] **Step 5: Run to verify pass** — as Task 1 Step 5.
- [ ] **Step 6: Commit** — `fix(tui): ctrl+c is the only escape while a start is cancelling`
...
- [ ] **Step 1: Fix the assertion** (`start_flow_test.go:576`) — flatten to
...
- [ ] **Step 2: Verify it actually fails when violated** — temporarily change `app.go:448`
...
- [ ] **Step 3: Run to verify pass** — as Task 1 Step 5.
- [ ] **Step 4: Commit** — `test(tui): make the AllowReplace assertion actually assert`

### Task 5: picker footer, stale comments, dead code

Findings 6 and 7.

- [ ] **Step 1: Footer** — `model_list.go:180`: the hint must distinguish the row actions,
...
- [ ] **Step 2: Comments** — `app.go:446` ("until Task 4's flip the legacy hint is still
...
- [ ] **Step 3: Dead code** — `handleStartMsg` returns `(model, tea.Cmd, bool)` whose `ok` is
