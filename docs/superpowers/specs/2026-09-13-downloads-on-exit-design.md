...
- **Cancellation (Ctrl+C).** The runner wraps `apply(...)` in `try/except KeyboardInterrupt`: on interrupt, call `pending.cancel()` (sets the existing `cancelled` flag → `aborted()` gates between every step; the current step finishes/terminates, remaining never start), print `Cancelled: N steps completed, M remaining skipped.`, exit non-zero. `apply()`'s own exception safety net (`PendingChanges._persist`, triggered whenever a real exception — including this KeyboardInterrupt — propagates out of the deletes/moves/ready/exposes loops) still persists every step that fully completed before the interrupt landed; only the interrupted step itself and anything not yet started are lost. Reuses the `cancelled`/`aborted()` machinery. Provider subprocesses are NOT reliably killed by interrupt signal delivery alone — `_tracked_popen_runner` (ollama.py) terminates its tracked subprocess itself, in its own `except BaseException` handler, the moment the KeyboardInterrupt lands inside `proc.wait()`; `pending.cancel()`'s own `cancel_current()` call (in `run_queued_ops`'s except block) is a secondary safety net that by then usually finds nothing left to do.
