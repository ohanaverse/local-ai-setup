...
`modelman` is a small Python 3.13 Textual TUI and CLI for managing local LLM models across multiple providers (Ollama, oMLX — llama.cpp is retired but its provider code is kept; see `docs/reference/provider-artifacts.md`) and exposing them through LiteLLM. **LiteLLM management is wt-owned (since 2026-09-21):** modelman no longer edits LiteLLM's `config.yaml` or restarts the proxy — it delegates to the `wt` binary (`wt litellm ...`) and **requires `wt` on PATH** (install with `make install`; write paths raise a clear error before changing any state when it is missing). The TUI lets you browse models, queue changes (ready/delete/move/expose), and apply them on exit.
...
| `expose <id>` / `unexpose <id>` | Apply modelman's gates, then delegate to `wt litellm expose|unexpose` (wt writes the `model_list` row and restarts the proxy) |
| `litellm status\|on\|off\|set` | Passthroughs to `wt litellm status\|on\|off\|set` (wt owns the routing state) |
...
- `src/modelman/main.py` defines the Typer `app`. A single `@app.callback(invoke_without_command=True)` opens the TUI when no subcommand is given; subcommands: `migrate` (one-shot legacy import), `sync` (reconcile state against providers), `expose <model_id>` / `unexpose <model_id>` (gates here, route write delegated to `wt litellm`), `delete-family <name>` (removes an empty family's lingering `[[families]]` entry left behind by queue.py's stickiness — the only remaining way to do so now that FamilyScreen is gone; refuses if the family still has models), `start <model_id>` / `stop` (issue #65 local-model lifecycle — see `local_control.py` below). Three sub-Typer apps are also mounted: `app.add_typer(benchmark_app, name="benchmark")`, `app.add_typer(usage_app, name="usage")`, and `app.add_typer(litellm_app, name="litellm")`.
- `main.py::litellm_app` (mounted as `modelman litellm`) — thin passthroughs to `wt litellm status|on|off|set [--url --api-key]` via `wt_bridge` (`status` prints wt's own text; wt never prints the api key). wt owns the `[litellm]` routing state in its `~/.config/agent-wt/config.toml` (migrated once from modelman.toml's legacy table), so modelman keeps no routing logic and never starts, stops or restarts the proxy. The TUI's LiteLLM status line and `l` toggle use the same bridge ("LiteLLM: unavailable" when wt cannot be reached; the mount-time status read uses a 5s subprocess timeout (`STATUS_TIMEOUT`), ~11 ms in practice; other bridge calls keep the 120s default).
...
- `src/modelman/litellm.py` — no longer writes LiteLLM's config (the ruamel writer, `save_litellm_config`, `ensure_litellm_settings()`, `restart_litellm_proxy()`, the `MODELMAN_LITELLM_RESTART_CMD` handling and the `PROVIDER_POLICIES` table were removed 2026-09-21; wt's `internal/litellm` owns all of it, including the launcher-required settings and the proxy restart). What remains: read helpers for `modelman usage` (`load_litellm_config`, `_database_url_from_config`, `_reverse_model_index`), the display predicates (`is_effectively_exposed`, `passes_ready_gate`, `is_cloud`, `is_cloud_effective`, backed by `wt litellm providers` through `wt_bridge.provider_cloud_flags()`; a `ProviderPolicy(cloud=...)` is only a minimal view of that; with wt missing they degrade to "unmapped/non-cloud" with a one-time stderr warning), and the thin delegating `expose_model`/`unexpose_model`/`apply_expose_queue`/`apply_unexpose_queue`. Those apply modelman's own gates against its IN-MEMORY state (model/provider exist, not native, provider mapped, ready or cloud), call `wt litellm expose|unexpose --json --skip-ready-gate`, and flip the `exposed` flag in modelman.toml only after wt reports success; `expose_model`/`unexpose_model` return wt's warning strings so the CLI/TUI can surface them. A mixed TUI apply queue (exposes plus unexposes) can cost up to two proxy restarts.
- `src/modelman/wt_bridge.py` — subprocess wrapper around `wt litellm ...` (`expose`, `unexpose`, `routed_ids`, `provider_cloud_flags`, `litellm_status[_text]`, `litellm_set_enabled`, `litellm_set`); parses the `--json` schemas (contract fixture `docs/contracts/litellm-cli.sample.json`), raises `WtNotFoundError`/`WtBridgeError`, and never puts argv (which may carry `--api-key`) into an error message. Sets `WT_LITELLM_CONFIG` for the child when a caller passes a config path.
- `src/modelman/_toml_io.py` — shared atomic-write helpers: `atomic_write()` (temp file + rename; `binary`/`preserve_mode` options — `preserve_mode=True` keeps a file's permission bits across rewrites), `atomic_write_toml()` (registry/state TOML writes), plus `tomllib`/`tomli_w` shims (`drop_none`, `unknown_keys` for round-trip preservation).
...
`stop-all --keep` just forwards the raw string. `src/modelman/local_process.py`
is a neutral shared home for process/probe types used by both this package
and `benchmark/isolation.py` — neither owns the concept, so living under
either `benchmark/` or `providers/` would make the other's import backwards.
...
- `src/modelman/benchmark/runmeta.py` (`git_sha()`) and `errors.py`'s `RunSavedButRestoreFailed` — run helpers shared by the agent and eval runners (each previously carried its own copy).
- `src/modelman/benchmark/judge_core.py` — shared LLM-judge mechanics (strict-JSON rubric scoring, retry-on-malformed reply, multi-sample median-combine) generalized out of the agent benchmark's judge so both `benchmark/agent/judge.py` (fixed diff-fixing rubric, diff-specific anonymization/overclaim detection) and `benchmark/eval/` (a different `Rubric` per capability category) can reuse it.
- `src/modelman/benchmark/eval/` — the cross-category capability benchmark (`modelman benchmark eval`): single-turn generation + judge scoring per row, no pi driver, no gates. `category.py` (loads a category dir's `items.toml`/`rubric.toml`/`rubric.md`, except `coding`, which has no rubric), `suite.py` (TOML parsing + row expansion + preflight, deliberately parallel to `agent/suite.py` rather than sharing it), `judged_runner.py` (one generation call per item + a judge_core call, for the four judged categories — reasoning/planning/code_review/doc_summary), `evalplus_runner.py` (subprocess wrapper around the installed `evalplus.evaluate` console script for the `coding` category; reports the "+"/hardened pass@1 when EvalPlus prints one, not the base-only score), `runner.py` (provider-grouped isolation loop mirroring `agent/runner.py`, plus `rejudge_run` for cheap re-scoring from persisted `response.txt` without regenerating), `report.py` (summary.md + score.json/metrics.jsonl artifact writes), `cli.py` (`run`/`list-categories`/`list-items`/`show`/`judge`). See `docs/guides/11-capability-eval-benchmark.md`.
- Tests: `tests/benchmark/agent/` (one file per module above, plus `fixtures/fake_agent.py` and `fixtures/tasks/mini-drift/`).
- Tests: `tests/benchmark/` (cli, errors, isolation, results, runner, state_pointer, workload_registry, workloads — one file per module above; also `test_judge_core.py` for the shared judge module).
- Tests: `tests/benchmark/eval/` (one file per `eval/` module above, plus `test_rejudge.py` for `rejudge_run`).
...
- Tests redirect config/registry/state paths via `MODELMAN_REGISTRY`, `MODELMAN_STATE` (new) and the legacy `MODELMAN_CONFIG` / `MODELMAN_FAMILY_DIR` (still honored by `migrate` tests). LiteLLM config paths redirect via `MODELMAN_LITELLM_CONFIG` (read by `modelman usage`'s reader; the `wt` child also accepts it as a legacy alias for `WT_LITELLM_CONFIG`); the LiteLLM DSN via `MODELMAN_LITELLM_DATABASE_URL`.
...
- `PendingChanges.apply()` is the single integration point between the TUI queue and the providers. It deliberately reorders to deletes-before-downloads so a queued delete frees disk before a queued download fills it, and runs exposes last so any queued download is visible to wt (which reads the registry/state from disk). The delete step now also checks `provider.is_downloaded()` first: when the artifact is already gone, the provider's `delete()` is skipped but registry/state cleanup and lifecycle events still run — so the TUI's `d` action can queue a delete for any model, ready or not.
...
- Config-file rewrite behavior (permission bits, atomic write, comment-preserving yaml.v3 with normalized list indentation and dropped blank lines on the first write, flock) now lives in wt's `internal/litellm` — see `wt/CLAUDE.md`'s "LiteLLM routes" section. modelman tests stub the `wt` bridge (conftest autouse guards) instead of writing config.yaml.
...
modelman reader (the TUI's RUNNING column, `modelman start`'s other-running
warning) verifies it with a live probe first. wt no longer reads the flag
at all — it probes the providers itself and decides launch/start/block
from those live rows (see `wt/CLAUDE.md`'s "Local-model resolution"
section).

`start_local_model()` also exposes the model (flips `exposed=True` and has wt write its LiteLLM `model_list` entry via `wt litellm expose`, mirroring `modelman expose`; note `wt start` itself also adds the started model's route, so a model started with wt is routable without modelman's flag) as part of every start — including the idempotent already-running path — so wt's picker, which no longer checks `exposed` for local models (see `wt/CLAUDE.md`'s Exposure predicate), still works for agents whose route to it is forced through LiteLLM; a failure to expose degrades to a warning rather than blocking the start. `stop_local_model()`/`stop_all_local_models()` symmetrically un-expose (wt removes the LiteLLM `model_list` row and restarts the proxy) any model that was exposed — `exposed` is no longer sticky across a stop/start cycle (2026-09-17): a stopped model must not stay routable through a dead backend, and a later start always re-exposes anyway. `_clear_stale_running_flag()` (the self-heal path used when a probe finds a flagged-running model actually dead — reached from `start_local_model`'s occupant-replacement/idempotency checks and from `running_model_ids()`, which the TUI's on-mount reconcile calls) does the same best-effort un-expose, so a model modelman no longer believes is running stops being routable there too, not just on an explicit stop. All of this is best-effort: a failed unexpose degrades to a warning (CLI) or a notify (TUI) rather than blocking the stop. See `docs/superpowers/specs/2026-09-15-wt-local-model-visibility-design.md` (predates this change; describes the original sticky-exposed design).

The TUI's `s` keybinding always confirms before starting or stopping — even a solo start with nothing else running — since either operation can be slow (a multi-GB warmup, a proxy restart) and disruptive; the stop dialog also names the un-expose side effect. A busy-guard (`ModelScreen._lifecycle_worker_busy()`) rejects a second `s` press outright while a start/stop worker (`run_worker(..., thread=True)`, named `model-start`/`model-stop`) is still in flight, rather than queuing or racing it. Start/stop still run as background thread workers — not literally blocking the event loop — so Textual keeps repainting/toasting during a long warmup; Textual cannot cancel one once started, so a real warmup can block that thread for minutes. Quitting (`ModelScreen.action_back()`, reached by both Escape and Ctrl+Q) checks `self.workers` for anything still `is_running` (model start/stop, the on-mount reconcile, the daily price refresh) and, if so, shows `ConfirmForceQuitDialog` instead of exiting normally: `Keep waiting`, or `Force quit`, which calls `ModelScreen._force_quit()` to restore the terminal and then `os._exit(0)` — the only way out, since a normal exit would otherwise hang in `asyncio.run()`'s executor-join and then `concurrent.futures.thread`'s untimed `atexit` thread-join waiting for that same thread. Safe by the hint-not-truth `running` contract above: an abandoned start just leaves a flag the next live probe self-heals.
...
The TUI's models screen (`ModelScreen`) now does this discovery too, not
just `modelman start` — `discover_unregistered_models()` runs alongside
the on-mount reconcile worker and renders each discovered artifact as a
synthetic `[cyan]+[/cyan]` row the user can register from the table (see
the STATUS glyph note above). See
`../docs/superpowers/specs/2026-09-15-modelman-tui-discovery-design.md`
(monorepo-root docs).
