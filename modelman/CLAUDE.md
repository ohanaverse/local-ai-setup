...
- `src/modelman/screens/__init__.py` — `reload_preserving_cursor(table, repopulate)` helper: snapshots the row key under the cursor before `DataTable.clear()` (which resets to row 0) and restores the cursor onto that key after repopulate. Both `FamilyScreen.reload()` and `ModelScreen._load_models()` route through it. `reconcile_model_state(models, registry, state)` is the shared reconcile write-path both screens' background workers delegate to (see below) — groups by provider and calls `provider.list_local()` at most once per provider, and only when at least one model in that provider's batch is ready (never unconditionally — that reintroduces the same per-reconcile subprocess cost `list_local()`-per-model had, just moved up a level).
- `src/modelman/screens/families.py` — `FamilyScreen`: DataTable of families (family · display · variants · downloaded · size). Actions: `action_add_family` (`a`), `action_edit_family` (`e`, renames display_name only — the slug is read-only to avoid orphaning cross-references), `action_delete_family` (`d`, blocked if any downloaded models), `action_open_family` (Enter / `DataTable.RowSelected`), `action_quit` (`q`). Reconcile runs automatically on mount/`on_screen_resume` — there is no manual `r` binding (that key is `toggle_ready` on the model screen). While `_reconciling` is true the table is disabled, a "Refreshing sizes…" indicator is shown, and every action above (plus `RowSelected`) no-ops so the user can't click a row that's about to mutate; `q` (quit) stays live. The `DOWNLOADED` column counts only models whose `location` is local (or unset/empty, for legacy entries) AND that reconcile verified on disk (wrote `state.ready=True`); `cloud` and other non-local locations are excluded from both the count and the `SIZE` total.
- `src/modelman/screens/models.py` — `ModelScreen`: single DataTable (family · provider · model · loc · status · exposed · cost · subscription · size) plus a details panel Static below the table showing the row's on-disk path (`path: —` when unknown). LOC is an icon (↗ cloud / ▤ local / `—` when unknown) and EXPOSED renders `Y`/`–`; there is no PATH column. **EXPOSED is the AND of the exposure flag and readiness** — a flagged but not-ready model renders `–` until apply finishes the queued download (the user's `x` press on a not-ready model cascades a `ready=True` queue, see below; the column flips to `Y` once the apply run completes and reconcile writes `state.ready=True`). **Cloud models are exempt from the readiness gate** — same `is_cloud()` check `_validated_entry` uses at the apply gate — so a flagged cloud row always renders `Y`. STATUS renders queued ops as glyphs with priority `✗ delete > ↓ download > → move > ✓ ready > ○`. Holds `queued_ready` / `queued_deletes` / `queued_moves` / `queued_exposes` dicts. Actions: `a` add model (family selectable), `e` edit (id/provider fixed; picking a different family queues a move), `d` queue delete (works on any model — the old not-ready gate is gone; apply skips the on-disk removal if `provider.is_downloaded()` reports False), `r` toggle ready (queues download/pull for reconcilable providers, or a flag flip for cloud/native; a no-op with a notification for a local-artifact model already on disk — reconcile is the only writer of `ready=False` for those, so delete the file instead), `x` toggle exposed (cascades a ready=True queue first if the model isn't ready yet — this cascade is what makes the EXPOSED column flip to `Y` after apply, since the column reads from `state.ready`, not the queued dict), `escape` back/apply. Cancelling either half of a cascade (pressing `r` or `x` a second time back to the persisted value) cancels the other half too — only when that half was itself cascade-added, not when the user queued it independently — so cancelling never leaves an expose queued against a model that will end up not-ready, or a download queued for an expose the user un-queued. `_provider_list()` returns `sorted(...)` so the Add dialog's provider dropdown is alphabetical. The `LOC` icon and the family-screen `DOWNLOADED` count use the same location rule: only `local` (or unset/legacy empty) counts as on-disk; `cloud` entries are never counted as downloaded.
- Both `FamilyScreen` and `ModelScreen` run a background worker (`_run_reconcile`, `thread=True`) on mount and on `on_screen_resume` — there is no manual reconcile binding; it runs automatically. Both workers delegate their actual state-write logic to the shared `reconcile_model_state()` (`screens/__init__.py`, see above) rather than duplicating it — `FamilyScreen` passes all `registry.models`, `ModelScreen` passes `registry.models_by_family(self.family)`. For a local-artifact model (per `registry.model_has_local_artifact` — driven by `ModelEntry.location`/`ProviderEntry.location`, not a hard-coded provider list), it writes `state.ready`/`disk_path`/`size_bytes` directly from what the provider reports; for a cloud-located or cloud-provider model, `ready` is left alone (only `disk_path`/`size_bytes` are opportunistically updated) since reconcile cannot verify a remote model against a local filesystem. A known `state.disk_path` is preserved unless reconcile observes a fresh one. `FamilyScreen` wraps the worker in try/finally + `app.call_from_thread(self._reconcile_done)` so the lock always clears, even on worker exception.
...
- **FamilyScreen** `DOWNLOADED` counts only local models and only sums their sizes; `cloud` entries are excluded even if the provider reports them present.
