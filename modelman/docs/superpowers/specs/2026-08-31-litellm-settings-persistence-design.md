...
3. **Curated settings are owned, not just preserved.** modelman knows the
   ...
   | `use_chat_completions_url_for_anthropic_messages: true` | `litellm_settings` (global) | LiteLLM's `/v1/messages` endpoint (claude-wt's wire protocol) bridges any `openai`-provider deployment through the OpenAI Responses API by default; mtplx/omlx/mlx_lm_server (all `openai/<name>`) implement only `/v1/chat/completions`, so every real request 404s and the deployment gets cooldown-blacklisted (discovered debugging mtplx 2026-09-16) | **value-enforced**: set to `true` when missing or not `true`. LiteLLM only exposes this globally, not per-deployment, so unlike the presence-based `litellm_params` rules below it cannot be scoped to just the `openai/*` rows that need it — accepted since every current `openai/*` deployment is a local backend needing it |
   ...
   | `use_chat_completions_api: true` | entry `litellm_params` | codex calls LiteLLM's native `/v1/responses` endpoint directly, which assumes an `openai`-provider deployment speaks the real OpenAI Responses API; mtplx/omlx/mlx_lm_server/llamacpp (all `openai/<name>`) implement only `/v1/chat/completions`, so every real request 404s and the deployment gets cooldown-blacklisted into a 429 (discovered debugging `wt smoke` codex×mtplx 2026-09-16) | **presence-based**, added 2026-09-16: add when missing to rows whose `litellm_params.model` starts with `openai/` *and* whose `litellm_params.api_base` is a loopback host — `openai/` is also LiteLLM's own syntax for a real OpenAI cloud deployment, so the `api_base` check is what scopes this to modelman's own local backends rather than a hand-added real one sharing the prefix; an existing key (any value) is left untouched |

   The two value-enforced/presence-based families differ deliberately. `drop_params` exists solely for the
