...
def test_wait_for_port_closed_read_timeout_means_still_open():
    ...
    with (
        ...
        wait_for_port_closed("http://localhost:8003/v1/models", timeout=0.3)
...
def test_port_closed_within_returns_false_on_timeout():
    ...
    fail, when a stop didn't fully release its port."""
    with (
        patch(
            "modelman.providers.lifecycle.probe.urllib.request.urlopen",
            side_effect=TimeoutError,
        ...
        assert port_closed_within("http://localhost:8003/v1/models", timeout=0.05) is False
...
def test_wait_for_port_open_returns_false_after_timeout():
    ...
    to decide whether to proceed or warn."""
    with (
        patch(
            "modelman.providers.lifecycle.probe.urllib.request.urlopen",
            side_effect=ConnectionRefusedError(),
        ...
        assert (
            wait_for_port_open("http://localhost:8000/v1/models", timeout=0.05, interval=0.01)
            is False
        )
...
def test_warmup_fails_when_no_chat_completion_marker():
    ...
    out (here the deadline is made immediate by mocking time)."""
    body = b'{"object":"list"}'
    with (
        patch("modelman.providers.lifecycle.probe.urllib.request.urlopen", _mock_urlopen(body)),
        patch("modelman.providers.lifecycle.probe.time.monotonic", side_effect=[0.0, 1.0, 1000.0]),
        patch("modelman.providers.lifecycle.probe.time.sleep"),
...
def test_warmup_retries_while_health_url_unreachable():
    ...
    misread as a chat-completion failure."""
    ...
    with (
        ...
        warmup("http://x/chat", "org/repo", health_url="http://x/health", timeout=1.0)
    # Only the health URL should ever have been polled.
