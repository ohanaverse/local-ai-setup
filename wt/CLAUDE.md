...
- Usage history (1d/7d/30d per-model counts) lives at `~/.config/agent-wt/usage.jsonl` (JSONL, appended by `usage.Store.Record`, consumed by the model picker — see `internal/usage`). The picker's `buildModelItems` (`internal/tui/model_list.go`) receives a `familyOf` map built from the agent's **full** catalog (`cfg.ModelsForAgent`) and runs ONE `Store.Counts` pass over those full-catalog IDs, then aggregates per-family totals in memory via `usage.AggregateByFamily`. That keeps family 30-day counts accurate even when `-T`/`-F` filters narrow the eligible slice. It then sorts eligible models descending by family-then-model `CompositeScore` (a recency-weighted integer key) and renders each model as one compact line — the family name and its 30-day count lead every row; there are no divider/header rows, family context is inline.
