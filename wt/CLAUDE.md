...
**Concurrent writers (issue #143).** `config.toml` has two independent writers after startup: the `wt config` editor (`internal/configeditor`, owns Agents/DefaultTag) and `wt litellm on|off|set` (`Config.UpdateLitellm`, owns `[litellm]`). Both persist through `(*Config).PatchSave(apply func(fresh *Config))`, which serializes on an flock (`WithLock`, `config.toml.lock`) and re-reads config.toml fresh under the lock before letting `apply` mutate only the fields it owns — a long-lived editor session's stale in-memory snapshot can therefore never revert a concurrent `wt litellm` change (or vice versa). `Save(cfg)` is the plain whole-file writer (also lock-guarded); use it only when cfg is known-fresh (just read in the same call, as `Load`'s one-time schema-migration writes do) — anything holding a possibly-stale `Config` across an await/goroutine/process boundary should go through `PatchSave` instead.
